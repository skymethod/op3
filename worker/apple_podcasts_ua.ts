...
export type ApplePodcastsUserAgent = { appName: string, appVersion: string, cfVersion: string, dwVersion: string, trailer: string | undefined, cfVersionExtra: string | undefined };
...
export function parseApplePodcastsUserAgent(userAgent: string): ApplePodcastsUserAgent | undefined {
    ...
    const m = /^([^/\s]+)\/(\d+)(\.(\d+))?(\.(\d+))?(\.(\d+))? CFNetwork\/(\d+)(\.(\d+))?(\.(\d+))?(\.(\d+))?(\.(\d+))? Darwin\/(\d+)\.(\d+)\.(\d+)( (\(x86_64\)( [0-9A-Za-z]{6})?|Opendium|[^\s]+))?$/.exec(userAgent);
    ...
    const [ _, appName, 
        ...
        cfVersion1, _cfVersion2, cfVersion2, _cfVersion3, cfVersion3, _cfVersion4, cfVersion4, _cfVersion5, cfVersion5,
    ...
    const cfVersionExtra = typeof cfVersion5 === 'string' ? pad(cfVersion5) : undefined;
    return { appName, appVersion, cfVersion, dwVersion, trailer, cfVersionExtra };
}
