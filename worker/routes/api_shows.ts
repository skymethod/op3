...
export async function computeShowsResponse({ showUuid, method, searchParams, rpcClient, roRpcClient }: { showUuid: string, method: string, searchParams: URLSearchParams, rpcClient: RpcClient, roRpcClient?: RpcClient }): Promise<Response> {
    ...
    const { results: showRecords = [] } = await targetRpcClient.adminExecuteDataQuery({ operationKind: 'select', targetPath: `/show/shows/${showUuid}` }, DoNames.showServer);
