...
export function findUserAgentEntity(userAgent: string): Entity | undefined {
    ...
    if (/AccountingPodcastDownloader\//.test(userAgent)) return { name: `AccountingPodcastDownloader`, type: 'bot' };
    ...
    if (/^amazon-Quick-on-behalf/.test(userAgent)) return { name: `Amazon Quick`, type: 'bot' };
    ...
    if (/^atlas-transcribe\//.test(userAgent)) return { name: `atlas-transcribe`, type: 'bot' };
    ...
    if (/^audio-sanity-check\//.test(userAgent)) return { name: `audio-sanity-check`, type: 'bot' };
    if (/ Audioscrape\//.test(userAgent)) return { name: `Audioscrape`, type: 'bot' };
    ...
    if (/^beehiiv-podcast-importer\//.test(userAgent)) return { name: `Beehiiv podcast importer`, type: 'bot' };
    ...
    if (/^ConduitMCP\//.test(userAgent)) return { name: `ConduitMCP`, type: 'bot' };
    if (/^Podcast-CriticalMention\//.test(userAgent)) return { name: `Critical Mention`, type: 'bot' };
    if (/ Crowli\//.test(userAgent)) return { name: `Crowli`, type: 'bot' };
    ...
    if (/^DutchPodcastScraper\//.test(userAgent)) return { name: `DutchPodcastScraper`, type: 'bot' };
    ...
    if (/^fetch-container\/\d/.test(userAgent)) return { name: `Fetch Container`, type: 'bot' };
    ...
    if (/ kidou-downloader\//.test(userAgent)) return { name: `kidou-downloader`, type: 'bot' };
    if (/^KutPod-Importer\//.test(userAgent)) return { name: `KutPod Importer`, type: 'bot' };
    ...
    if (/^LinuxGetUrl\/\d/.test(userAgent)) return { name: `LinuxGetUrl`, type: 'bot' };
    ...
    if (/^podcast-downloader$| podcast-downloader\//.test(userAgent)) return { name: `Podcast Downloader`, type: 'bot' };
    if (/^PodcastFeedAnalyzer\//.test(userAgent)) return { name: `PodcastFeedAnalyzer`, type: 'bot' };
    if (/^Mozilla.* PodcastFetcher\//.test(userAgent)) return { name: `PodcastFetcher`, type: 'bot' };
    if (/^podcast-go-audio-downloader\//.test(userAgent)) return { name: `Podcast Go Audio Downloader`, type: 'bot' };
    if (/ PodcastIndexer\/\d/.test(userAgent)) return { name: `PodcastIndexer`, type: 'bot' };
    if (/ PodcastTranslator\/\d/.test(userAgent)) return { name: `PodcastTranslator`, type: 'bot' };
    if (/^Podcast Transcriber \d/.test(userAgent)) return { name: `Podcast Transcriber`, type: 'bot' };
    ...
    if (/^(PodcastDL|podcastdl)\//.test(userAgent)) return { name: `PodcastDL`, type: 'bot' };
    ...
    if (/^podcast-download-worker\//.test(userAgent)) return { name: `podcast-download-worker`, type: 'bot' };
    ...
    if (/^podcast-preflight\//.test(userAgent)) return { name: `podcast-preflight`, type: 'bot' };
    ...
    if (/pods3-backfill/.test(userAgent)) return { name: `pods3-backfill`, type: 'bot' };
    ...
    if (/^PodSearcher\/| PodSearcher\//.test(userAgent)) return { name: `PodSearcher`, type: 'bot' };
    ...
    if (/public-episode-downloader/.test(userAgent)) return { name: `public-episode-downloader`, type: 'bot' };
    ...
    if (/Reference Finder Podcast Downloader/.test(userAgent)) return { name: `Reference Finder Podcast Downloader`, type: 'bot' };
    ...
    if (/^sciis-media-fetch\//.test(userAgent)) return { name: `Sciis media fetch`, type: 'bot' };
    ...
    if (/^slopsearch\//.test(userAgent)) return { name: `slopsearch`, type: 'bot' };
    ...
    if (/^truename-podcast-transcriber\//.test(userAgent)) return { name: `truename podcast transcriber`, type: 'bot' };
    ...
    if (/^UniversalFeedParser\//.test(userAgent)) return { name: `UniversalFeedParser`, type: 'bot' };
    ...
    if (/^vercel-image-optimization\//.test(userAgent)) return { name: `Vercel image optimization`, type: 'bot' };
    ...
    if (/ WhisperProxy\//.test(userAgent)) return { name: `WhisperProxy`, type: 'bot' };
    if (/^ZDM\/.*Windows/.test(userAgent)) return { name: `Windows Crawler`, type: 'bot' };
    if (/(Windows| )PowerShell\//.test(userAgent)) return { name: `Windows PowerShell`, type: 'bot' };
    ...
    if (/^ARD Sounds|^ARD%20Sounds|^ARD-Sounds/.test(userAgent)) return { name: `ARD Sounds`, type: 'app' };
    ...
    if (/^Herd\//.test(userAgent)) return { name: `Herd`, type: 'app' };
    ...
    if (/^Micropod\//.test(userAgent)) return { name: `Micropod`, type: 'app' };
    ...
    if (/^Pinepods\//.test(userAgent)) return { name: `Pinepods`, type: 'app' };
    ...
    if (/^Skimr\//.test(userAgent)) return { name: `Skimr`, type: 'app' };
    ...
    if (/^UpNext\//.test(userAgent)) return { name: `Up Next`, type: 'app' };
    ...
    if (/ ExoPlayerLib\/|^Exo2\//.test(userAgent)) return { name: `ExoPlayer (Android)`, type: 'library' };
