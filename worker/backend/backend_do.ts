        ...
        try {
            ...
            if (method === 'POST' && pathname === '/rpc') {
                try {
                    ...
                    } else if (obj.kind === 'execute-sql' && DoNames.isHlsInstance(durableObjectName)) {
                        return newRpcResponse({ kind: 'execute-sql', ...await getOrLoadHlsInstanceController().executeSql(obj) });
                    } else {
