...
export class CombinedRedirectLogController {
    ...
    async adminExecuteDataQuery(req: Unkinded<AdminDataRequest>, backupBlobs: Blobs | undefined): Promise<Unkinded<AdminDataResponse>> {
        ...
        if (targetPath === '/crl/records/backup') {
            const { storage } = this;
            const attNums = await this.getOrLoadAttNums();
            const computeHeaderLine = () => JSON.stringify(attNums.toJson());
            const computeRecordLine = (key: string, record: DurableObjectStorageValue) => {
                ...
                return attNums.removingPackedAtt(record, 'encryptedIpAddress');
            }
            const computeKeyRange = (item: string) => {
                // crl.r.<timestamp>.<uuid>
                const [ timestampStart, timestampEnd ] = [0, 1].map(v => computeTimestamp(`${addHoursToHourString(item, v)}:00:00.000Z`).substring(0, 8)); // yymmddhh
                return { startKeyInclusive: `crl.r.${timestampStart}`, endKeyExclusive: `crl.r.${timestampEnd}` };
            }
            const isValidItem = isValidHour;
            const prefix = 'hits/1/';
            return await this.backups.execute({ operationKind, parameters, backupBlobs, isValidItem, prefix, computeHeaderLine, computeKeyRange, storage, computeRecordLine });
        }
